  ...
  _buildHandlers() {
    ...
    this._handlers = {
      ...
      Split: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        if (!focusNodeWindow) return;
        // forge-t1s9: a detached focus node (no parentNode) throws on the deref
        // below. Bail like the float toggle and WindowResetSizes do.
      ...
      ShowTabDecorationToggle: (action, ctx) => {
        ...
        if (!focusNodeWindow) return;
        if (!wm.ext.settings.get_boolean("tabbed-tiling-mode-enabled")) return;
        // Unlike the handlers above this one does not throw on a detached node — it
        // assigns `attachNode = null`, which leaves the next render placing from a
        // null cursor. Same guard, different symptom.
        if (!focusNodeWindow.parentNode) return;
