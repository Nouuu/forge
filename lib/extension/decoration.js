  ...
  hideWindowBorders() {
    ...
    nodeWindows.forEach((nodeWindow) => {
      ...
      if (nodeWindow.parentNode && nodeWindow.parentNode.isStackedOrTabbed()) {
        // forge-5r0j: `_destroyed` is a flag Forge never sets, so guarding on it was
        // vacuous — a finalized St actor throws on the first property read instead.
        // hideWindowBorders runs on every render and from the minimize handlers, so
        // an escaping throw aborts the rest of the pass. Same try/catch shape as
        // Tree._destroyTab; the next render rebuilds the tab.
        try {
          if (nodeWindow.tab && nodeWindow.tab.get_parent()) {
            nodeWindow.tab.remove_style_class_name("window-tabbed-tab-active");
          }
        } catch (e) {
          Logger.debug(`hideWindowBorders: skipping finalized tab: ${e}`);
        }
      }
    });
  }
