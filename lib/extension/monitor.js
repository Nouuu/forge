...
export class MonitorManager extends GObject.Object {
  ...
  addMonitor(wsIndex) {
    ...
    for (let mi = 0; mi < monitors; mi++) {
      ...
      // forge-fhen.10: use the shared guarded accessor like every other site in lib/.
      // get_monitor_geometry can return null for an index that vanished mid
      // monitors-changed, even while get_n_monitors still counts it.
      let monitorRect = Utils.getMonitorGeometrySafe(mi);
