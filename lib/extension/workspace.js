...
export class WorkspaceManager extends GObject.Object {
  ...
  addWorkspace(wsIndex) {
    ...
    // Resolve the Meta.Workspace BEFORE building anything. Mutter returns null for
    // an index whose workspace is already gone (a removal racing the signal, a stale
    // index during dynamic-workspace teardown), and the old order created the node,
    // its layout and its St.Bin — parenting the bin into window_group — and only then
    // took the `!workspace` exit. That reported failure while leaving a monitor-less,
    // signal-less workspace node and a live bin behind: the forge-98sa leak shape, on
    // the add side. Nothing removes it, because removeWorkspace is only ever called
    // for workspaces the tree believes in.
    let workspace = wsManager.get_workspace_by_index(wsIndex);
    if (!workspace) {
      this._tree.removeChild(newWsNode);
      return false;
    }
  ...
  /**
   * Rewrite `workspace-skip-tile` through the same index shift the tree nodes take.
   *
   * The setting stores raw indices, so without this an insert or a remove silently
   * re-points the exclusion at a DIFFERENT workspace — routinely, since GNOME's
   * dynamic workspaces add and remove them as you work. The tree already follows the
   * same Meta.Workspace as it shifts (`_renumberWorkspaces`); this keeps the user's
   * choice attached to the same workspace too.
   *
   * @param {(idx: number) => number|null} remap - New index for an entry, or null to
   *   drop it (its workspace is gone).
   */
  _renumberSkipList(remap) {
    const settings = this._extWm.ext?.settings;
    if (!settings) return;
    const raw = settings.get_string("workspace-skip-tile");
    ...
    const next = raw
      ...
      .map((entry) => {
        ...
        // A hand-edited non-numeric entry means nothing to us; pass it through
        // rather than silently eating part of the user's setting.
        if (isNaN(idx)) return entry;
    ...
    // Only write on a real change: this runs from a workspace signal, and the write
    // re-enters _onSettingsChanged.
    if (joined !== raw) settings.set_string("workspace-skip-tile", joined);
  }
  ...
  renumberWorkspacesAfterRemoval(removedIndex) {
    ...
    this._renumberSkipList((idx) => {
      if (idx === removedIndex) return null; // its workspace is gone
  ...
  renumberWorkspacesAfterAddition(insertedIndex) {
    ...
    this._renumberSkipList((idx) => (idx >= insertedIndex ? idx + 1 : idx));
  }
