...
export class FocusManager extends GObject.Object {
  ...
  warpPointerToNodeWindow(nodeWindow) {
    ...
    if (newCoord && newCoord.x && newCoord.y) {
      const seat = Compat.getDefaultSeat();
      if (seat) {
        ...
        try {
          ...
          // A warp failure is cosmetic; this runs inside the focus handler, so
          // letting it escape would break focus itself.
          Logger.warn(`warp_pointer failed: ${e}`);
        }
      }
    }
  }
  ...
  _focusWindowUnderPointer() {
    ...
    const settings = this._extWm.ext.settings;
    if (!settings) {
      // Same hazard as the guard above: returning false self-destructs the source,
      // so the id must be zeroed or the next pointerLoopInit/_removeSignals calls
      // GLib.Source.remove on a dead id. Reached between disable() nulling
      // ext.settings and an already-scheduled source firing.
      this._extWm._pointerFocusTimeoutId = 0;
      return false;
    }
    ...
    // The loop ticks every 16 ms, so a motionless pointer would otherwise drive
    // ~62 focus()+raise() calls per second at each rest. Nothing changes when the
    // hovered window already holds focus, so skip the pair entirely.
