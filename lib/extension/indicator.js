...
export class FeatureMenuToggle extends QuickMenuToggle {
  ...
  constructor(extension) {
    ...
    // `title` is the GNOME 45+ QuickMenuToggle property (it replaced `label`), but
    // the @girs typings only describe the inherited Actor props, so the cast is
    // needed. It was previously hidden: the removed isGnomeGTE(45) ternary produced
    // a union of two shapes, which degraded inference enough that tsc never checked
    // this call. metadata.json declares shell-version 45..50, so the pre-45 arm was
    // unreachable.
    super(/** @type {any} */ ({ title, iconName, toggleMode: true }));
    this.extension = extension;
...
export class FeatureIndicator extends SystemIndicator {
    ...
    this._settingsChangedId = this.extension.settings.connect("changed", (_, name) => {
      switch (name) {
        ...
        case "tray-icon-enabled":
          // forge-5r0j: `_destroyed` is a flag Forge never assigns, so guarding on
          // it was vacuous — a finalized St actor throws on the first property
          // access instead of reporting itself. This runs from a GSettings signal,
          // so an escaping throw lands in the shell's main loop.
          try {
            ...
            if (this._indicator) this._indicator.visible = tiling && quick && tray;
          } catch (e) {
            Logger.debug(`FeatureIndicator: skipping finalized indicator: ${e}`);
          }
      }
    });
  }
  ...
  destroy() {
    ...
    // Optional chaining on settings: destroy() runs from disable(), which nulls
    // ext.settings on its way out, and GNOME also destroys Quick Settings items on
    // session-mode changes — a bare deref would throw out of the shell's teardown.
    if (this._settingsChangedId) {
