...
export const LAYOUT_TYPES = createEnum(["STACKED", "TABBED", "ROOT", "HSPLIT", "VSPLIT"]);
...
export class Node extends GObject.Object {
  ...
  removeChild(node) {
    ...
    if (!refNode) {
      // A real Error, not a bare string: callers on the render/destroy paths
      // (tree.js removeNode/flatten, window.js untrackWindow) let this escape, and
      // a thrown string carries no stack, so the abort surfaces unexplained.
      throw new Error(`NodeNotFound ${node}`);
    }
    return refNode;
  }
    ...
    if (this.isWindow()) {
      ...
      if (value) {
        ...
        if (
          ...
          floatAlwaysOnTop &&
          !isFullscreen &&
          // forge-zo4: never re-apply a pin the fullscreen reconcile deliberately
          // suspended. processFloats re-derives `float = true` on every render, so
          // without this the pin was re-applied and re-dropped once per render for
          // as long as a fullscreen window was up. Ownership is already claimed on
          // a demoted float, so there is nothing to record here.
          !metaWindow._aboveDemotedForFullscreen
        ) {
          metaWindow.make_above();
          metaWindow._forgeSetAbove = true; // Track that Forge set this
        }
      } else {
        this.mode = Window.WINDOW_MODES.TILE;
        // Only remove always-on-top if Forge was the one who set it
        if (metaWindow.is_above() && metaWindow._forgeSetAbove) {
...
export class Tree extends Node {
  ...
  split(node, orientation, forceSplit = false) {
    ...
    let parentNode = node.parentNode;
    // The type check above admits MONITOR/CON/WINDOW without verifying parentNode, so
    // a detached or stale node of one of those types threw here instead of taking a
    // null exit like the guards above it (forge-clsp).
    if (!parentNode) return null;
  ...
  decorationLayout(rectY, height, barSize, position) {
    if (position === "bottom") {
      // Clamp to rectY: processStacked places the bar column with the UNCAPPED
      // totalBars, so a column taller than the container would anchor above the
      // container's own top edge. Overflow belongs below, exactly as it does for
      // "top" — i3 squeezes the content, never the header column. Inert for a
      // capped barSize, which is always <= height - 1.
      return { contentY: rectY, decorationY: Math.max(rectY, rectY + height - barSize) };
    }
  ...
  applyMargins(rect) {
    ...
    // Clamp at 0: the gschema declares these as plain `u` with no <range>, so a
    // direct gsettings or settings.json write can exceed the work area even though
    // the prefs spinners cap each at 100 px. A negative rect falls straight through
    // processGap's `nodeWidth > gap * 2` guard and makes Tree.apply skip every
    // child, so windows keep stale geometry with nothing logged.
    return {
      x: rect.x + marginLeft,
      y: rect.y + marginTop,
      width: Math.max(rect.width - marginLeft - marginRight, 0),
      height: Math.max(rect.height - marginTop - marginBottom, 0),
    };
  }
  ...
  _applyDecorationRect(node, child, params, barSize, tabExpand) {
    ...
    try {
      ...
      // Inside the try on purpose: Node.render() derefs this.tab.get_child_at_index(1),
      // so a finalized tab throws here too. Left outside, that throw escaped
      // processStacked/processTabbed -> processNode -> Tree.render and aborted the rest
      // of the pass; renderTree's finally only resets the source id. Destroying the
      // decoration self-heals the tab it came from, so the same recovery applies.
      child.render();
    } catch (e) {
