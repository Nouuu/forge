 ...
 * ids, the stacked-raise debounce source id, and the three always-on-top markers.
 *
 * The pin-ownership flags live HERE and not on the tree node on purpose: a node is
 * destroyed and rebuilt by every tree reload (`Tree.reload` empties `childNodes`),
 * while the GNOME pin they describe lives on the Meta.Window and survives it. Parked
 * on the node, `_forgeSetAbove` was lost on each reload — after which Forge could
 * neither unpin its own float nor tell it apart from a pin the user applied.
 * @typedef {Meta.Window & {
 *   windowSignals?: number[],
 *   _forgeStackTimeoutId?: number,
 *   _forgeTransientAbove?: boolean,
 *   _forgeSetAbove?: boolean,
 *   _aboveDemotedForFullscreen?: boolean,
 * }} ForgeMetaWindow
 */
/**
...
export class WindowManager extends GObject.Object {
  ...
  _bindSignals() {
    ...
    // Monitor hot-plug. The signal comes from Main.layoutManager, NOT from
    // MetaDisplay (forge-0rb6) — connecting it on global.display throws.
    this._layoutManagerSignals = [
  ...
  /**
   * Monitor configuration changed: rebuild the tree's `mo{m}ws{n}` scaffold.
   *
   * A full reload rather than an incremental add/remove, on purpose. `reloadTree`
   * already recreates the monitor nodes at the live monitor count, tears down the
   * stale St.Bins (Tree._removeScaffoldBins) and restores stacked/tabbed groupings
   * (forge-bqa) — an incremental `syncMonitors` would duplicate all three for an
   * event that fires when someone plugs in a cable. It is also debounced by
   * `_reloadTreeSrcId`, so Mutter's burst of signals during one reconfiguration
   * collapses into a single reload.
   *
   * Without this the scaffold went stale until an unrelated reload: a window moved
   * onto a new monitor was never re-homed (updateMetaWorkspaceMonitor needs the
   * `mo{m}ws{n}` node) and an unplugged monitor's nodes and bins stayed live.
   */
  _onMonitorsChanged() {
    // Same guard as _onWorkareasChanged: Mutter transiently reports zero monitors
    // on a KVM switch, lock or DPMS off. Reloading there would wipe the tree and
    // strand every tracked window for a monitor loss that is about to be undone.
    if (global.display.get_n_monitors() === 0) {
      Logger.debug(`monitors-changed: no monitors, ignoring signal`);
      return;
    }
    this.reloadTree("monitors-changed");
  }
  ...
  cleanupAlwaysFloat() {
    ...
    this._forEachFloatNode((metaWindow) => {
      ...
      // The pin is gone, so the flags that describe it must go too. A stale
      // _forgeSetAbove makes _restoreAllDemotedFloats re-pin the window the user
      // just unpinned, and makes a later unfloat strip a pin the USER owns.
      metaWindow._forgeSetAbove = false;
      metaWindow._aboveDemotedForFullscreen = false;
    });
  }

  restoreAlwaysFloat() {
    this._forEachFloatNode((metaWindow) => {
      // Bug #289 parity with the `float` setter: a fullscreen window is never
      // pinned, or it renders above the panel and every dialog.
      if (metaWindow.is_fullscreen()) return;
      // forge-zo4: a float the reconcile demoted keeps its pin SUSPENDED while the
      // fullscreen window is up. Claim it — so the reconcile can put it back once
      // that window goes — but do not re-apply it here, or toggling this setting
      // during a fullscreen session lifts the float straight back over the
      // fullscreen surface. Found by the ownership fuzzer.
      if (metaWindow._aboveDemotedForFullscreen) {
        metaWindow._forgeSetAbove = true;
        return;
      }
      if (metaWindow.is_above()) return;
      metaWindow.make_above();
      // Claim the pin. Without this, _reconcileFullscreenFloatDemotion and
      // `set float(false)` — both of which require _forgeSetAbove — cannot see it,
      // so it can be neither demoted under a fullscreen window nor unfloated away.
      metaWindow._forgeSetAbove = true;
    });
  }
   ...
   * per-window `_aboveDemotedForFullscreen` flag carries the "restore me once my
   * monitor has no fullscreen window" intent. Unlike cleanupAlwaysFloat — which
   * unpins every float, dialogs included — this pass excludes dialogs and transients
   * (_isDialogLike) and only ever touches floats Forge itself pinned.
   */
    ...
    this._withSuppressedAboveHandler(() => {
      nodes.forEach((w) => {
        ...
        if (
          ...
          metaWindow.lower();
          metaWindow._aboveDemotedForFullscreen = true;
          return;
        }

        // Restore: a previously-demoted float whose monitor is now clear.
        if (!blocked && metaWindow._aboveDemotedForFullscreen) {
          if (metaWindow._forgeSetAbove && !metaWindow.is_above()) metaWindow.make_above();
          metaWindow._aboveDemotedForFullscreen = false;
        }
      });
    });
  ...
  _restoreAllDemotedFloats() {
    ...
    this._withSuppressedAboveHandler(() => {
      this.allNodeWindows.forEach((w) => {
        const metaWindow = w.nodeValue;
        // forge-h7ba/forge-olv3: a finalized wrapper is still truthy, and this loop
        // runs from disable(), from the setting-off reconcile exit and from
        // windowDestroy — the same synchronous pre-prune window the sibling loops
        // were hardened for. Probe before touching the flag: on a dead wrapper even
        // the expando read is not worth trusting.
        if (!Utils.isWindowAlive(metaWindow)) return;
        if (!metaWindow._aboveDemotedForFullscreen) return;
        if (metaWindow._forgeSetAbove && !metaWindow.is_above()) metaWindow.make_above();
        metaWindow._aboveDemotedForFullscreen = false;
      });
    });
  ...
  disable() {
    ...
    this.allNodeWindows.forEach((node) => {
      ...
      if (mw._forgeTransientAbove) {
        try {
          if (Utils.isWindowAlive(mw) && mw.is_above() && !mw._forgeSetAbove) {
  ...
  get windowsAllWorkspaces() {
    ...
    // forge-h7ba family: drop finalized wrappers BEFORE the sort. On a fast/Wayland
    // close the wrapper can finalize while the window is still listed, and
    // get_stable_sequence() throws on it — inside a comparator, so the throw escapes
    // the getter itself. Both callers are teardown-critical: _removeSignals (a throw
    // there aborts disable() mid-way and leaves every remaining window connected,
    // the bug-328 storm) and trackCurrentWindows (a throw there aborts a whole
    // re-track, which reloadTree drives on enable, workspace reorder and monitor
    // hot-plug). A dead wrapper has nothing left to track or disconnect anyway.
    windowsAll = windowsAll.filter((w) => Utils.isWindowAlive(w));
  ...
  getWindowsOnWorkspace(workspaceIndex) {
    ...
    // The ws node is genuinely absent during the destroy-then-reload window
    // workspaces-reordered opens. Returning null here is what floatWorkspace /
    // unfloatWorkspace already expect — their `if (!workspaceWindows) return`
    // guards could never fire while this threw first.
  ...
  toggleWorkspaceMonocle() {
    ...
    // Scope to the monitor holding the FOCUSED window, not to monitorNodes[0].
    // i3/sway scope fullscreen per output, and taking the first node meant that on
    // a multi-monitor workspace the toggle appeared to do nothing from the second
    // screen while quietly re-tabbing the first. Focus, not pointer: this is a
    // keyboard action, and with move-pointer-focus off the pointer can be resting
    // on another screen entirely. Falls back to the pointer's monitor, then to the
    // first node (a stale scaffold, or an index Mutter reports before the tree has
    // caught up).
    const monitorNodes = workspaceNode.getNodeByType(NODE_TYPES.MONITOR);
    if (!monitorNodes || monitorNodes.length === 0) return;

    const focusMon = this.focusMetaWindow?.get_monitor?.();
    const focusMonNode =
      focusMon != null && focusMon >= 0
        ? this.tree.findNode(Utils.createMonitorWorkspaceId(focusMon, workspaceIndex))
        : null;
    const monitorNode = focusMonNode ?? this.currentMonWsNode ?? monitorNodes[0];
  ...
  move(
    ...
    //
    // This is the ONLY place the flag is consumed, and deliberately so: Tree.apply
    // used to clear it for every tiled child, including ones whose move() had
    // early-returned (a live grab, a degenerate renderRect). Those windows lost the
    // one-shot without ever being placed, so the next render stripped the very
    // effect this guard exists to keep.
  ...
  _removeSignals() {
    ...
    disconnectSignals(Main.layoutManager, this._layoutManagerSignals);
    this._layoutManagerSignals = null;
   ...
   */
  _handleUserAboveChange(metaWindow) {
    ...
    // Reaching here means the USER toggled the pin. If it is gone, so is Forge's
    // ownership of it: a stale _forgeSetAbove would make a later user re-pin read
    // as Forge's own (so isFloatingExempt would stop treating it as an overlay)
    // and would let a later unfloat strip a pin Forge does not own — the case
    // FR-003 and bug-319 exist to prevent.
    if (metaWindow && !metaWindow.is_above()) {
      metaWindow._forgeSetAbove = false;
      metaWindow._aboveDemotedForFullscreen = false;
    }
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        // `??=`, not `=`: only a window Forge has never seen gets the Bug #530
        // one-shot. tree.reload() empties the tree, so trackCurrentWindows re-enters
        // this branch for EVERY live window — enable, workspaces-reordered, the
        // no-meta-monws fallback and monitor hot-plug all land here. Re-arming the
        // flag there hands the skip to windows that are not new, so their next real
        // placement leaves an in-flight shell effect running instead of cancelling
        // it. A consumed flag is `false`, which `??=` preserves.
            ...
            if (this.floatingWindow(focusNodeWindow)) {
              this.queueEvent({
                ...
                callback: () => {
                  ...
                  const fw = /** @type {ForgeMetaWindow | null} */ (this.focusMetaWindow);
                  const fwNode = fw ? this.tree.findNode(fw) : null;
                  if (fw && fwNode && this.floatingWindow(fwNode) && !fw._aboveDemotedForFullscreen)
                    fw.raise();
        ...
        if (existNodeWindow.parentNode && metaMonWsNode) {
          ...
          if (!containsWindow) {
            ...
            if (this.isActiveWindowWorkspaceTiled(metaWindow)) {
              ...
              if (this.floatingWindow(existNodeWindow) && !metaWindow._aboveDemotedForFullscreen) {
   ...
   * @param {number} [removedIndex=-1] - When called from the workspace-removed path,
   *   the index being removed; -1 (the idle reconcile) means indices already agree.
   */
  _rehomeWindowToLiveLocation(wNode, removedIndex = -1) {
    ...
    if (!ws) return;
    // forge-ojew: on the workspace-removed path this runs BEFORE
    // renumberWorkspacesAfterRemoval, so Mutter has already renumbered while the tree
    // has not. A live index at or past the doomed one maps back a slot. Only k == 0
    // actually collides (for k > 0 Mutter relocates to l->prev at k-1, which never
    // renumbers), but the mapping is uniform and a no-op for the idle reconcile.
    const liveIndex = ws.index();
    const treeIndex = removedIndex >= 0 && liveIndex >= removedIndex ? liveIndex + 1 : liveIndex;
    const destNode = this.tree.findNode(
      Utils.createMonitorWorkspaceId(metaWindow.get_monitor(), treeIndex)
    );
  ...
  _rehomeWorkspaceWindowsBeforeRemoval(wsIndex) {
    ...
    for (const wNode of windowNodes) {
      ...
      try {
        this._rehomeWindowToLiveLocation(wNode, wsIndex);
      } catch (e) {
  ...
  _onSettingsChanged(settingName) {
    ...
    switch (settingName) {
      case "window-overrides-reload-trigger":
        ...
        // The rules are only consulted during a render (processFloats ->
        // isFloatingExempt -> _classifyTileOverrides), so refreshing the cache
        // without one leaves an edited rule inert until an unrelated event.
        this.renderTree(settingName);
        break;
      // forge-l64o: every key below is read only DURING a render (margins in
      // tree.js applyMargins, the rest in window.js/decoration.js), so a key
      // missing from this group stays invisible in prefs until an unrelated
      // event happens to re-render.
      case "focus-border-toggle":
      case "focus-border-hidden-on-single":
      case "window-margin-top":
      case "window-margin-right":
      case "window-margin-bottom":
      case "window-margin-left":
      case "window-maximize-on-single":
      case "split-border-toggle":
      case "focus-border-radius":
      case "showtab-decoration-enabled":
        this.renderTree(settingName);
        break;
  ...
  _handleGrabOpEnd(_display, _metaWindow, grabOp) {
    ...
    this.grabOp = null;
    // Bug #151's pointer snapshot is per-grab state as well: getDragPointer()
    // treats "pointer still at the snapshot" as a touch/stylus drag, so a
    // snapshot surviving the grab can misclassify the next one.
    this._grabStartPointer = null;
  }
  ...
  /**
   * Whether a window's title matches an override's wmTitle value. Shared by the
   * float and the tile matchers, which used to disagree: tile did a plain
   * case-sensitive `includes`, so docs/user/rules.md documented one semantic the
   * two paths did not both implement.
   *
   * forge-11k: titles are matched by substring, never exact, so lowercasing both
   * sides is safe and gives locale/casing fidelity for titles like
   * "Picture-in-Picture". A single space is the exception — it means "match the
   * literal one-space title" and stays an equality test.
   */
  _wmTitleMatches(overrideWmTitle, windowTitle) {
    if (!overrideWmTitle) return false;
    if (overrideWmTitle === " ") return overrideWmTitle === windowTitle;
    if (!windowTitle) return false;
    const haystack = windowTitle.toLowerCase();
    return overrideWmTitle
      .split(",")
      .some((t) =>
        t.startsWith("!")
          ? !haystack.includes(t.slice(1).toLowerCase())
  ...
  _classifyTileOverrides(metaWindow) {
    ...
    for (const override of allOverrides) {
      ...
      if (override.wmTitle) {
        matchTitle = this._wmTitleMatches(override.wmTitle, windowTitle);
      }
      if (override.wmClass) {
  ...
  _matchesFloatRule(kf, metaWindow) {
    ...
    if (kf.wmTitle) {
      matchTitle = this._wmTitleMatches(kf.wmTitle, windowTitle);
    }
    if (kf.wmClass) {
  ...
  isFloatingExempt(metaWindow) {
    ...
    let floatByRole =
      ...
      // Skip Forge's own always-on-top float pin too: reading it back as a user
      // overlay made the decision self-fulfilling, so a window that floated for a
      // TRANSIENT reason (null wm_class at map time, a float rule the user deleted,
      // a workspace removed from workspace-skip-tile) could never re-tile —
      // processFloats kept re-deriving float=true from the pin it had set itself.
      (metaWindow.is_above() && !metaWindow._forgeTransientAbove && !metaWindow._forgeSetAbove) ||
  ...
  // Bug #319: both loops go through the float/tile setters, like
  // floatWorkspace/unfloatWorkspace. Assigning `mode` directly skipped the
  // make_above/unmake_above reconciliation, and nothing caught up afterwards:
  // these run from TilingModeToggle, and renderTree short-circuits to
  // decoration-only while tiling-mode-enabled is false, so processFloats never
  // runs to fix the stacking.
  floatAllWindows() {
