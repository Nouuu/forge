...
export class WindowManager extends GObject.Object {
  ...
  toggleWorkspaceMonocle() {
    ...
    // Scope to the monitor the user is ON, not to monitorNodes[0]. i3/sway scope
    // fullscreen per output, and taking the first node meant that on a multi-monitor
    // workspace the toggle appeared to do nothing from the second screen while
    // quietly re-tabbing the first. Falls back to the first node when the current
    // monitor has none (a stale scaffold, or a monitor index Mutter reports before
    // the tree has caught up).
    const monitorNodes = workspaceNode.getNodeByType(NODE_TYPES.MONITOR);
    if (!monitorNodes || monitorNodes.length === 0) return;

    const monitorNode = this.currentMonWsNode ?? monitorNodes[0];
