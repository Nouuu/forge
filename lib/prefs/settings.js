  ...
  constructor({ settings, aboutButton }) {
    ...
    this.add_group({
      ...
      children: [
        ...
        new SwitchRow({
          title: _("Focus on Hover only while tiling"),
          subtitle: _("Suspends focus-follows-pointer when tiling is off"),
          settings,
          bind: "focus-on-hover-tiling-only",
        }),
        new SwitchRow({
    ...
    this.add_group({
      ...
      description: _(
        "Disables tiling on specified workspaces. Starts from 0, separated by commas. " +
          "Indices follow workspaces as they are added or removed"
      ),
      children: [
        new EntryRow({
